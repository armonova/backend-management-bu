...
beforeEach(async () => {
  ...
  video = await Video.create({ user })
})

test('POST /videos 201 (user)', async () => {
  const { status, body } = await request(app())
    .post(`${apiRoot}`)
    .send({ access_token: userSession, intrument: 'test', year: 'test', championship: 'test', link: 'test' })
  expect(status).toBe(201)
  expect(typeof body).toEqual('object')
  expect(body.intrument).toEqual('test')
  expect(body.year).toEqual('test')
  expect(body.championship).toEqual('test')
...
test('PUT /videos/:id 401 (user) - another user', async () => {
  const { status } = await request(app())
    .put(`${apiRoot}/${video.id}`)
    .send({ access_token: anotherSession, intrument: 'test', year: 'test', championship: 'test', link: 'test' })
  expect(status).toBe(401)
})
